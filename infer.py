...
def main(args):
    ...
    for result in results:
        ...
        if prompt not in binary_masks:
            print(f"Skipping save for '{prompt}' — no mask generated (no heatmap points found)")
            continue
        save_path = os.path.join(args.save_dir, f"output_{prompt}_{image_name}.png")
        print(f"  Saving per-prompt figure: {save_path}")
    ...
    seg_path = os.path.join(args.save_dir, image_name + "_segmented.png")
    mask_path = os.path.join(args.save_dir, image_name + "_mask.png")
    heatmap_path = os.path.join(args.save_dir, image_name + "_heatmap.png")

    Image.fromarray(final_image).save(seg_path)
    Image.fromarray(final_mask.astype(np.uint8)).save(mask_path)
    Image.fromarray((heatmap_normalized * 255).astype(np.uint8)).save(heatmap_path)
